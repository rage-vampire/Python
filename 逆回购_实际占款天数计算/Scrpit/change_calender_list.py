...
def get_trading_cal():
    dates = []
    tradeing_value = []
    with open('../trade_datas.csv', 'r') as csvfile:
        reader_file = csv.reader(csvfile)
        for row in reader_file:
            dates.append(row[0])
            tradeing_value.append(row[1])
    return dates, tradeing_value


# def get_tradeing_cal():
#     dates = []
#     trade_type = []
