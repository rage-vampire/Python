...
class Convert:
    ...
    def filter_data(self):
        for data in self.orig_data:
            ...
            if re_new_order is not None:
                self.msgtype_1xxx01()
            if re_order_cofirm is not None:
                self.msgtype_2xxx02()
            if re_order_match is not None:
                self.msgtype_2xxx15()
            if self.int_msg_type == 190007:
                self.msgtype_190007()
            if self.int_msg_type == 290008:
                self.msgtype_290008()
    ...
    def msgtype_1xxx01(self):
        ...
        self.int_msg_type = self.int_msg_type  # 获取MsgType的值
        ...
        all_data = f'{str(self.timestamp).strip()}', f'{str(self.int_msg_type).strip()}', '', '', \
...
# def AsciiTohex(self, to_hex):
#     """将ASCII转换成十六进制"""
#     list_str = []
#     for i in to_hex:
#         list_str.append(hex(ord(i))[2:])
#     hex_data = ''.join(list_str)
#     self.logger.info(f"ascii转换成hex后的值为：{hex_data}")
#     return hex_data
